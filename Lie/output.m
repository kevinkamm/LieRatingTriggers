...
% SDE P
[latexFilePath,latexCommand]=saveRatingProperties(mDCcal,sDDcal,dMLcal,iRScal,rSOcal,'SDE','ratingPropertiesP');
fprintf(inputFile,...
        '\t\\input{%s}\n',changeSlash(latexFilePath));
for iLC=1:1:length(latexCommand)
    fprintf(inputFile,...
...
% SDE Q
[latexFilePath,latexCommand]=saveRatingProperties(mDCcalQ,sDDcalQ,dMLcalQ,iRScalQ,rSOcalQ,'SDE','ratingPropertiesQ');
fprintf(inputFile,...
        '\t\\input{%s}\n',changeSlash(latexFilePath));
for iLC=1:1:length(latexCommand)
    fprintf(inputFile,...
...
if ~isempty(figRDP)
    ...
    latexFilePath=saveFigures(figRDP,'RD',['RD_P']);
    fprintf(inputFile,...
            '\t\\input{%s}\n',changeSlash(latexFilePath));
end
if ~isempty(figRDQ)
    fprintf(inputFile,...
            '\\subsection{Plots of Distributions}\n');
    latexFilePath=saveFigures(figRDQ,'RD',['RD_Q']);
    fprintf(inputFile,...
            '\t\\input{%s}\n',changeSlash(latexFilePath));
end
...
if ~isempty(figTraP)
    ...
    latexFilePath=saveFigures(figTraP,'Tra',['Tra_P']);
    fprintf(inputFile,...
            '\t\\input{%s}\n',changeSlash(latexFilePath));
end
if ~isempty(figTraQ)
    fprintf(inputFile,...
            '\\subsection{Plots of Trajectories}\n');
    latexFilePath=saveFigures(figTraQ,'Tra',['Tra_Q']);
    fprintf(inputFile,...
            '\t\\input{%s}\n',changeSlash(latexFilePath));
end
%% Close file
