...
PDdataset=1;
[PD,~,~]=defaultProbabilityLoader('Data/PD','Fitch',PDdataset,months./12);
...
fileName = sprintf('GEM_%s_N%d_M%d',optimizer,N,M);
saveCal = true;
loadCal = true;

disp('Start calibration under P')
[params,errCal,dWP,t,tInd,ctimeCal]=calibrateP(months,T,N,M,Rgan,...
                                    'Optimizer',optimizer,...
                                    'saveCal',saveCal,...
                                    'loadCal',loadCal,...
...
%% Nested SSA
% Nssa=1000*12+1; % for intermediate transitions we need a finer grid for the nested SSA than for the simulation 
% dtSSA=T/(Nssa-1);
% dt=T./(N-1);
% M1=10;
% M2=5000;
% i0=3;
% tic;
% X=nestedSSA(AQ./dt,t,i0,M1,M2,dtSSA);
% toc;
% % figure();
...
% figure();
% hold on;
% plot(linspace(0,T,N),squeeze(RcalQ(i0,end,:,w1)),'r-');
% % plot(linspace(0,T,Nssa),squeeze(mean(Xtemp==4,2)),'b--');
% stairs(linspace(0,T,Nssa),squeeze(mean(Xtemp==4,2)),'g--');
%% Output
compileLatex=false;
output(fileName,...
       N,M,...
