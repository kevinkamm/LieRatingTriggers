...
%% Calibration PHCTMC with exponential change of measure
disp('Start optimization Exp')
comType='Exp';
ticFmin=tic;
[AP,AQ,hFmin,errFmin,ctimeLsqnonlin,UPcal,UQcal]=calibrate(Padjusted,tMarket,PD,muQ,muP,comType);
ctimeFmin=toc(ticFmin);
fprintf('Finished optimization after %3.3f seconds\n',...
        ctimeFmin)
%% Calibration PHCTMC with JLT change of measure
disp('Start optimization JLT')
comType='JLT';
ticFminJLT=tic;
[APJLT,AQJLT,hFminJLT,errFminJLT,ctimeLsqnonlinJLT,UPcalJLT,UQcalJLT]=...
    calibrate(Padjusted,tMarket,PD,muQ,muP,comType);
...
fprintf('Error for Evo Sys Analytic of rating transitions under P: %1.3e\n',...
        sum(errAnalyticPMarket,'all'));
errAnalyticQPD=mean(abs(squeeze(UQcal(:,end,:))-PD),1);
fprintf('Error for Evo Sys Analytic of default probabilities under Q: %1.3e\n',...
...
fprintf('Error for Evo Sys of default probabilities under Q: %1.3e\n',...
...
fprintf('Error for Evo Sys of rating transitions under P: %1.3e\n',...
        mean(errEvoSysPMarket,'all'));

% JLT Measure
[UQjlt,~]=evoSys(dt,0,T,AQJLT,tMarket,@generatorPiecewise);
[UPjlt,~]=evoSys(dt,0,T,APJLT,tMarket,@generatorPiecewise);


%% Plot trajectories
% Measure P
traPlotP=plotTrajectories(t,UP,UPjlt);
% Measure Q
traPlotQ=plotTrajectories(t,UQ,UQjlt);
%% Sample ICTMC
M=10000;
...
% end
fprintf('Finished calculating probabilities of simulations after %1.3f s\n',...
        toc(ticSimProb));
%% Print errors of simulation
errSimPMarket=mean(abs(XPProbs(:,:,tk)-Padjusted),[1,2]);
fprintf('Error of simulated transition probabilities compared to market under P: %1.3e\n',...
        mean(errSimPMarket,'all'));
errSimPEvoSys=mean(abs(XPProbs(:,:,tk)-UP(:,:,tk)),[1,2]);
fprintf('Error of simulated transition probabilities compared to evolution system under P: %1.3e\n',...
        mean(errSimPEvoSys,'all'));
errSimQEvoSys=mean(abs(XQProbs(:,:,tk)-UQ(:,:,tk)),[1,2]);
fprintf('Error of simulated transition probabilities compared to evolution system under Q: %1.3e\n',...
...
%% Rating properties
% Exp change of measure
[mDCPExp,sDDPExp,dMLPExp,iRSPExp,rSOPExp]=ratingProperties(UPcal,tMarket*12);
[mDCQExp,sDDQExp,dMLQExp,iRSQExp,rSOQExp]=ratingProperties(UQcal,tMarket*12);
% JLT change of measure
[mDCPJLT,sDDPJLT,dMLPJLT,iRSPJLT,rSOPJLT]=ratingProperties(UPcal,tMarket*12);
[mDCQJLT,sDDQJLT,dMLQJLT,iRSQJLT,rSOQJLT]=ratingProperties(UQcal,tMarket*12);
%% Print Results
