function [AP,AQ,hFmin,errFmin,ctimeLsqnonlin,UP,UQ]=...
                                        calibrate(P,tMarket,PD,muQ,muP,comType)
...
if muP~=0
    ...
    for kk=1:1:length(tMarket)
        ...
        x0(:,1:end-1)=AP(:,:,kk);
        ticLSQ=tic;
        [xmin,err] = fmincon(@(x)fmin(x,AP(:,:,kk),PD(:,kk),UQt0,time(kk+1),time(kk),muQ,muP,comType),...
                     x0,...
                     [],[],...
                     [],[],...
                     lb,ub,...
        ...
        Ah=changeOfMeasure(AP(:,:,kk),hmin,'Type',comType);
        AQ(:,:,kk)=Ah;
